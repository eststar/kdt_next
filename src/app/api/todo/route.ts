...
export async function GET(request: NextRequest) {
    ...
    if(id){
        const idData = todoData.find((item : TodoData) => item.id === Number(id));
        if (idData)
            return NextResponse.json(idData);
        else
            return NextResponse.json(
                { error: `id: ${id}에 해당하는 자료가 없습니다.` },
                { status: 404 }
            );
    }
    else
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!completed) {
            return NextResponse.json({ error: "completed 파라미터가 필요합니다." }, { status: 400 });
        }
